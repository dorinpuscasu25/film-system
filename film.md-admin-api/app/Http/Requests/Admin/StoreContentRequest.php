...
class StoreContentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'seasons.*.episodes.*.bunny_library_id' => ['nullable', 'string', 'max:64', 'regex:/^\d+$/'],
            'seasons.*.episodes.*.bunny_video_id' => ['nullable', 'string', 'max:128'],
                ...
                $episodes = collect(data_get($season, 'episodes', []))
                    ->map(function (mixed $episode, int $episodeIndex): array {
                        ...
                        return [
                            ...
                            'bunny_library_id' => $this->filledArrayValue($episode, 'bunny_library_id'),
                            'bunny_video_id' => $this->filledArrayValue($episode, 'bunny_video_id'),
